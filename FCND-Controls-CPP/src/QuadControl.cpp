...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  //
    ...
    cmd.desiredThrustsN[1] = (-f_x + f_y - f_z + f) / 4.f;
    cmd.desiredThrustsN[2] = (f_x - f_y - f_z + f) / 4.f;
    cmd.desiredThrustsN[3] = (-f_x - f_y + f_z + f) / 4.f;
...
V3F QuadControl::BodyRateControl(V3F pqrCmd, V3F pqr)
{
  ...
  V3F momentCmd, rate_error, M_inertia ;
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    if (collThrustCmd > 0.0) {
        ...
        p_c = (R(1,0) * b_x_p_term - R(0,0) * b_y_p_term) / R(2,2);
        q_c = (R(1,1) * b_x_p_term - R(0,1) * b_y_p_term) / R(2,2);
